...
def test_snapshot_sets_truth_and_derives_current_from_pointer() -> None:
    ...
    assert result.state.current_id == 2  # trackIndex 3 -> current index 2 -> qid 2
...
def test_app_origin_remove_of_current_picks_successor() -> None:
    """Removing the current track advances current_id to the successor."""
    state = CanonicalState(
        cloud_version=QueueVersion(5, 1), tracks=_refs(0, 1, 2), current_id=1, active=True
    )
    result = reduce(
        state,
        CloudTracksRemoved(
